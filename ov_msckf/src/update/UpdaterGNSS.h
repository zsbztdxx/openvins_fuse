...
public:
  ...
  */
   ...
   */
  bool try_update(std::shared_ptr<State> state, const ov_core::SatNavData &gnss_data, const Eigen::Vector3d &init_lla,
              const Eigen::Matrix3d &init_R_GNSStoI, const Eigen::Vector3d &init_t_GNSStoI);

  /**
   * @brief Will first detect if the gnss pos is valid, then will update.
   * @param state State of the filter
   * @param ins_gnss_data inspvaa gnss data
   * @param init_lla lat,lon,alt transform to enu reference 
   * @param init_R_GNSStoI gnss enu position tranform Matrix to imu frame
   * @param init_t_GNSStoI gnss enu position translation to imu frame
   * @return True if the system is fuse gnss data
   */
  bool try_update(std::shared_ptr<State> state, const ov_core::InsPvaData &ins_gnss_data, const Eigen::Vector3d &init_lla,
              const Eigen::Matrix3d &init_R_GNSStoI, const Eigen::Vector3d &init_t_GNSStoI);
   ...
   * @param state State of the filter
   * @param ins_gnss_data inspvaa gnss data
   * @param rover_fix_level rover fix level of ggp
   * @param init_lla lat,lon,alt transform to enu reference 
   * @param init_R_GNSStoI gnss enu position tranform Matrix to imu frame
   * @param init_t_GNSStoI gnss enu position translation to imu frame
   * @return True if the system is fuse gnss data
   */
  bool try_update(std::shared_ptr<State> state, const ov_core::InsPvaData &ins_gnss_data, const int &rover_fix_level,
